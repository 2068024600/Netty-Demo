...
public class DecodeTools {
	...
	public static void decodeByBytes(ByteBuf in, List<Object> out) {
		...
		while (in.isReadable()) {
			// 读取包头
			if (in.readByte() == NettyClientConst.PACKAGE_HEAD) {
				// 记录包头的位置
				...
				try {
					...
					// 读取包长(包头下一位就是包长)
					String lenString = "";
					for (int i = 0; i < NettyClientConst.PACKAGE_LENGTH; i++) {
					...
					len = len - 2 - NettyClientConst.PACKAGE_LENGTH;
					// 判断当前读取的包长是否正确
					if (len <= 0) {
						/*
					...
					// 数据解析完毕，验证包尾
					if (in.readByte() == NettyClientConst.PACKAGE_END) {
	...
	public static void decodeByString(ByteBuf in, List<Object> out) {
		while (in.isReadable()) {
			// 判断包头第一位
			if (in.readByte() == NettyClientConst.HEAD_ONE) {
				// 在包头做个标记
				int position = in.readerIndex() - 1;

				try {
					// 判断后面是否还有数据 然后再次判断包头的第二位
					if (in.readByte() == NettyClientConst.HEAD_TWO) {
						// 标记，方便数据回滚
						in.markReaderIndex();
						// 获取包长
						String lenString = "";
						...
						int len = ConvertFactory.hexToInt(lenString);
						// 获取长度减掉包头、包长、包尾长度即可获取包体真实长度
						len = len - 4 - NettyClientConst.PACKAGE_LENGTH;
						// 判断包长是否大于0
						if (len <= 0) {
							// 小于0说明数据包有误，去掉包头再次解析
							in.resetReaderIndex();
						...
						// 读取包体后，判断包尾
						if (in.readByte() == NettyClientConst.END_ONE && in.readByte() == NettyClientConst.END_TWO) {
							// 当前数据包末尾还有数据，但是一个包已经解析完成
							if (in.isReadable()) {
								// 标记当前位置, 再次解析
								in.markReaderIndex();
