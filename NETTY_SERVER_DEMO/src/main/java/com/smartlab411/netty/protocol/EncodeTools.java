...
public class EncodeTools {
	...
	public static void encodeByBytes(DataFrame msg, ByteBuf out) {
		// 包头
		byte head = NettyServerConst.PACKAGE_HEAD;
		// 包长
		int length = msg.getLength() + 2 + NettyServerConst.PACKAGE_LENGTH;
		// 包体
		ByteBuf message = msg.getMessage();
		// 包尾
		byte end = NettyServerConst.PACKAGE_END;
		// 业务信息, 编码发送
		out.writeByte(head);
		out.writeBytes(ConvertFactory.intToByteArray(length, NettyServerConst.PACKAGE_LENGTH));
	...
	public static void encodeByString(DataFrame msg, ByteBuf out) {
		// 包头第一位字符
		byte headOne = NettyServerConst.HEAD_ONE;
		// 包头第二位字符
		byte headTwo = NettyServerConst.HEAD_TWO;
		// 包长
		String length = Integer.toHexString(msg.getLength() + 2 + NettyServerConst.PACKAGE_LENGTH).toUpperCase();
		// 转16进制字符串数字补全
		while (length.length() != NettyServerConst.PACKAGE_LENGTH) {
			length = "0" + length;
		}
		// 包体
		ByteBuf message = msg.getMessage();
		// 包尾第一位字符
		byte endOne = NettyServerConst.END_ONE;
		// 包尾第二位字符
		byte endTwo = NettyServerConst.END_TWO;
		// 业务信息, 编码发送
